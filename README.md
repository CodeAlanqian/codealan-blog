...
- 443 端口复用：  
  - 通过 `stream` 模块按 SNI 分流：  
    ...
    - `nas.codealan.top` → 8446 (独立 HTTPS 入口，反代到本机 5667)。
...
- NAS 反代：
  ...
  - 规划后端：本机 `http://127.0.0.1:5667`。
  - 该域名不接入 Hugo 博客，也不复用博客后端；它是独立 HTTPS 反代入口。
  - 在现有 443 SNI 分流结构下，建议新增内部 HTTPS 后端 `127.0.0.1:8446`，再由 Nginx stream 将 `nas.codealan.top` 分流到 8446，最后反代到本机 5667。
